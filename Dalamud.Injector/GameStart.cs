...
namespace Dalamud.Injector
{
    ...
    public static class GameStart
    {
        ...
        public static Process LaunchGame(GameStartContext context, Action<Process> beforeResume)
        {
            ...
            if (context.IsolationConfig?.Enabled == true)
                return LaunchGameIsolated(context, beforeResume);
            ...
            try
            {
                ...
                try
                {
                    Log.Information("Running legacy pinvoke");
        ...
        // This function should become the default, after some careful testing.
        private static Process LaunchGameIsolated(GameStartContext context, Action<Process> beforeResume)
        {
            unsafe
            {
                Process process = null;
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (context.IsolationConfig?.Enabled == true)
                        {
                            appContainer = CreateAppContainer(context);

                            capabilityList.Add(WELL_KNOWN_SID_TYPE
                                                   .WinCapabilityInternetClientServerSid); // for internet access
                            capabilityList.Add(WELL_KNOWN_SID_TYPE
                                                   .WinCapabilityPrivateNetworkClientServerSid); // for intranet access (except loopback)
                            ...
                            // TODO: CsWin32 generates this as internal for some reason
                            const nuint PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES = 131081U;
                            procThreadAttrList.Add(PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES, &capability, Marshal.SizeOf(capability));

                            startupInfoEx.lpAttributeList = procThreadAttrList.AsPointer();
                            Log.Information("Created appcontainer");
                        }
