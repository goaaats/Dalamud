...
internal interface IImGuiRenderer : IDisposable
{
    ...
    /// <summary>
    /// Draws and presents a single secondary (multi-viewport) window from a draw data pointer.
    /// </summary>
    /// <remarks>
    /// This mirrors what <c>ImGui.RenderPlatformWindowsDefault()</c> would do per secondary viewport
    /// (render then swap buffers), but using Dalamud-owned draw data and a renderer-private
    ...
    /// <param name="drawData">The draw data for the viewport.</param>
    void RenderViewport(nint rendererUserData, ImDrawDataPtr drawData);
}
